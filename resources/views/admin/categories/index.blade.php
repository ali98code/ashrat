                    ...
                    <div class="">
                        <div class="flex flex-wrap justify-between">
                            <div class="items-center ">
                                <h1 class="font-semibold text-xl mb-1 block dark:text-slate-100">التصنيفات</h1>
                                <ol class="list-reset flex text-sm">
                                    <li><a href="{{ route('admin.dashboard') }}" class="text-gray-500">لوحة التحكم</a></li>
                                    <li><span class="text-gray-500 mx-2">/</span></li>
                                    <li class="text-blue-600 hover:text-blue-700">التصنيفات</li>
                                </ol>
                            </div>
                            <div class="flex items-center">
                                <a href="{{ route('admin.categories.create') }}" class="px-3 py-2 lg:px-4 bg-blue-500 collapse:bg-green-100 text-white text-sm font-semibold rounded hover:bg-blue-600">إضافة</a>
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <table class="w-full" id="datatable_1">
                                ...
                                @foreach($categories as $category)
                                    <tr class="bg-white border-b border-dashed dark:bg-gray-800 dark:border-gray-700">
                                        ...
                                        <td class="p-3 text-sm text-gray-500 whitespace-nowrap dark:text-gray-400">
                                            @if($category->parent)
                                                <span class="bg-primary-500 text-white text-[11px] font-medium mr-1 px-2.5 py-1 rounded">{{ $category->parent->name }}</span>
                                            @else
                                                <span class="bg-gray-500 text-white text-[11px] font-medium mr-1 px-2.5 py-1 rounded">لا يوجد</span>
                                            @endif
                                        </td>
                                        <td class="p-3 text-sm text-gray-500 whitespace-nowrap dark:text-gray-400">
                                            {{ $category->slug }}
                                        ...
                                        <td class="p-3 text-sm text-gray-500 whitespace-nowrap dark:text-gray-400">
                                            <a href="{{ route('admin.categories.edit', $category->id) }}"><i class="ti ti-edit text-lg text-gray-500 dark:text-gray-400"></i></a>
                                            <a href="javascript:void(0)" onclick="delFunction({{ $category->id }})">
